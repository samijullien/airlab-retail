...
class Assortment():
    def __init__(self, size, seed=None):
        ...
        self.shelf_lives = torch.tensor(df.Shelf_life, dtype=torch.float32)
        self.dims = torch.tensor(df.iloc[:, 1:4].values)
        self.characs = torch.stack((self.selling_price, self.cost, self.shelf_lives)).t()
...
EnvInfo = namedtuple("EnvInfo", ["sales", "availability", "waste", "reward", "traj_done"])
...
class RetailTrajInfo(TrajInfo):
    ...
    def step(self, observation, action, reward, done, agent_info, env_info):
        ...
        self.AverageReward = self.Momentum * reward + (1 - self.Momentum) * self.AverageSales
        self.Sales = self.Momentum * getattr(env_info, "sales", 0) + (1 - self.Momentum) * self.AverageSales
        self.Availability = self.Momentum * getattr(env_info, "availability", 0) + (1 - self.Momentum) * self.Availability
        self.Waste = self.Momentum * getattr(env_info, "waste", 0) + (1 - self.Momentum) * self.Waste
...
class StoreEnv(Env):
    def __init__(self,
                 ...
                 clip_reward=False,
                 episodic_lives=True,  # keeping those for further integration
                 repeat_action_probability=0.,  # Probably going to stay 0 as repeating action does not seem like a very good idea
                 horizon=10000, 
                 seed=None,
                 substep_count = 4,
                 ...
                 lead_time = 1,
                 lead_time_fast = 0,
                 symmetric_action_space = False# Defines how quickly the orders goes through the buffer - also impacts the relevance of the observation
                 ):
        save__init__args(locals(), underscore=True)
        # Spaces
        if symmetric_action_space:
            self._action_space = FloatBox(low=-max_stock/2, high=max_stock/2, shape=[assortment_size])
        else:
            self._action_space = IntBox(low=0, high=max_stock, shape=[assortment_size])
        self.stock = torch.zeros(assortment_size, max_stock, requires_grad=False)
        # correct high with max shelf life
        self._observation_space = FloatBox(low=0, high=1000, shape=(assortment_size,max_stock+characDim+lead_time+lead_time_fast+1))
        self._horizon = int(horizon)
        self.assortment = Assortment(assortment_size, seed)
        self._repeater = torch.stack((self.assortment.shelf_lives, torch.zeros(self._assortment_size))).transpose(0,1).reshape(-1).detach()
        self.forecast = torch.zeros(assortment_size,1) # DAH forecast.
        self._step_counter = 0
        #Needs to move towards env parameters
        self._customers = d.multivariate_normal.MultivariateNormal(bucket_customers, bucket_cov)
        self.assortment.base_demand = self.assortment.base_demand.detach()/(bucket_customers.sum())
        ...
        self._year_multiplier = torch.arange(0., horizon, pi/365)
        self._week_multiplier = torch.arange(0., horizon, pi/7)  
        ...
        if utility_function=='linear':
            ...
            self.utility_function = CobbDouglas_Utility(**utility_weights)
        else:
            self.utility_function = Homogeneous_Reward(**utility_weights)
        self._updateEnv()
        for i in range (self._lead_time):
            self._addStock((self.forecast.squeeze()*bucket_customers[i]).round())
    ...
    def step(self, action):
        if self._symmetric_action_space:
            new_action = torch.as_tensor(action.round().clip(0, self._max_stock)+self._max_stock/2, dtype=torch.int32)
        else:
            new_action = torch.as_tensor(action, dtype=torch.int32).clamp(0,self._max_stock)
        if (self.day_position%self._substep_count == 0):
            order_cost = self._make_fast_order(new_action) 
            sales, availability = self._generateDemand(self.real.clamp_(0.,1.))
        ...
        info = EnvInfo(sales = sales, availability = availability, waste = waste, reward = utility, traj_done = done)
        return EnvStep(self.get_obs(), utility, done, info)

    def get_obs(self):
        return self._obs # copy the obs instead ? check that

    ###########################################################################
    # Helpers

    def _updateObs(self):
        self._obs = torch.cat((self.stock,self.assortment.characs, self.forecast, torch.stack(self._buffer + self._buffer_fast,1), torch.ones(self._assortment_size,1)*self.day_position),1)

    def _updateEnv(self):
        self.day_position = 1 
        argument = self._year_multiplier[self._step_counter] + self._phase
        argument2 = self._week_multiplier[self._step_counter] + self._phase2
        self.forecast = (self.assortment.base_demand *(argument).cos().abs() *(argument2).cos().abs()).view(-1,1)
        self.real = self.forecast.view(-1) + self._bias.sample((self._assortment_size,))
        self._updateObs()

    def _addStock(self, units):
        padding = self._max_stock-units
        replenishment = torch.stack((units, padding)).t().reshape(-1)
        restock_matrix = self._repeater.repeat_interleave(repeats = replenishment.long(), dim=0).view(self._assortment_size,self._max_stock)
        torch.add(self.stock.sort(1)[0],restock_matrix.sort(1, descending=True)[0], out = self.stock)
        total_units = restock_matrix.ge(1).sum(1).add_(self.stock.ge(1).sum(1))
        penalty_cost_forbidden = F.relu(total_units - self._max_stock).float().mul_(self.assortment.selling_price)
        return(penalty_cost_forbidden)

    def _sellUnits(self, units):
        sold = torch.min(self.stock.ge(1).sum(1).float(), units)
        availability = self.stock.ge(1).sum(1).float().div(units).clamp(0, 1)
        availability[torch.isnan(availability)] = 1.
        reward = sold.mul_(2).sub_(units).mul(self.assortment.selling_price - self.assortment.cost)
        ...
        binary_vec = torch.tensor([0.,1]).repeat(p).repeat_interleave(interleaver)
        self.stock = binary_vec.mul_(stock_vector).view(p,n)
        return reward, availability

    def _waste(self):
        waste = torch.mul(self.stock.eq(1).sum(1).float(), self.assortment.selling_price)
        return waste
    ...
    def _generateDemand(self, consumption_prob):
        # hourly customers
        sampled_customers = self._customers.sample().round().int()[self.day_position-1]
        purchases_gen = d.bernoulli.Bernoulli(consumption_prob)
        demand = purchases_gen.sample((sampled_customers,)).sum(0).clamp(0,self._max_stock)
    ...
    def transportation_cost(self, units, transport_size = 300000, transport_cost = 250.):
...
class Linear_Utility():
    ...
    def reward(self, sales, waste, availability):
...
class CobbDouglas_Utility():
    ...
    def reward(self, sales, waste, availability):
        return (sales**self._alpha)*((1+waste)**-self._beta)*(availability**self._gamma)
...
class LogLinear_Utility():
    ...
    def reward(self, sales, waste, availability):
        return (torch.log(1+sales)*self._alpha)-(torch.log(1+waste)*self._beta)+(torch.log(1+availability)*self._gamma)
